# Attack Dex #

## Agility ##

* Basic
* Bonus
* Utility
* Take an extra full movement.

## Barrier ##

* Advanced
* Full Turn
* Utility
* Concentration - Place a sixteen hex-edge, non cyclic wall on the field. The wall blocks movement and ranged attacks.

## Bite ##

* Basic
* Standard
* Melee
* Physical
* Dark
* Accuracy 20
* 4 Damage Dice
* Any extra effects

## Bug Bite ##

* Advanced
* Standard
* Melee
* Physical
* Bug
* Accuracy 20
* 4 Damage Dice

## Confuse Ray ##

* Advanced
* Full Turn
* Ranged
* Ghost
* Accuracy 20
* Concentration - Target is confused.

## Defense Curl ##

* Basic
* Reaction
* Utility
* Concentration - As a reaction to a physical attack, your defense is increased.

## Double Edge ##

* Advanced
* Standard
* Melee
* Physical
* Normal
* Accuracy 20
* 7 Damage Dice
* The user takes 1/3, rounded down, recoil damage

## Double Team ##

* Advanced
* Standard
* Utility
* Concentration - your evasion is 5.

## Ember ##

* Basic
* Full Turn
* Ranged
* Special
* Fire
* Accuracy 20
* 3 Damage Dice
* 105 burn chance to target

## Faint Attack ##

* Advanced
* Standard
* Melee
* Physical
* Dark
* 4 Damage Dice
* Faint Atack auto succeeds on accuracy rolls and does not allow for any evasion

## Fake Out ##

* Advanced
* Reaction
* Melee
* Physical
* Normal
* Accuracy 20
* 3 Damage Dice
* React to a melee attack agains you. Causes flinching.

## Flame Wheel ##

* Basic
* Standard
* Melee
* Special
* Fire
* Accuracy 20
* 4 Damage Dice

## Focus Energy ##

* Advanced
* Full Turn
* Utility
* Concentration - All your attacks that hit count as critical hits.

## Foresight ##

* Advanced
* Bonus
* Utility
* Concentration - You can hit ghosts with normal and fighting moves until the start of your next turn.

## Growl ##

* Basic
* Standard
* Utility
* All enemies in line of sight have reduced attack.

## Gust ##

* Basic
* Standard
* Ranged
* Special
* Flying
* Accuracy 20
* 3 Damage Dice

## Helping Hand ##

* Advanced
* Reaction
* Utility
* Add your special and physical attack stats to an ally's stats during an attack if you are in line of sight to both ally and target.

## Howl ##

* Basic
* Standard
* Utility
* Concentration - User has increased attack

## Hypnosis ##

* Advanced
* Standard
* Ranged
* Psychic
* Accuracy 12
* Concentration - Target goes to sleep.

## Imprison ##

* Advanced
* Bonus
* Utility
* Any opposing Poke'mon in line of sight can not use attacks that you have equiped.

## Leer ##

* Basic
* Standard
* Ranged
* Accuracy 20
* Concentration - Target has reduced defence.

## Nightshade ##

* Advanced
* Standard
* Ranged
* Ghost
* Accuracy 20
* Does damage equal to the user's level.

## Peck ##

* Basic
* Standard
* Melee
* Physical
* Flying
* Accuracy 20
* 3 Damage Dice

## Poison Sting ##

* Basic
* Bonus
* Melee
* Physical
* Poison
* Accuracy 20
* 1 Damage Dice
* 30% poisoning chance

## Protect ##

* Advanced
* Reaction
* Utility
* Provides complete immunity to an attack. The second time a pokemon uses Protect in a battle, it has a 4 in 8 chance of failure, 6 in 8 for a third use, and 7 in 8 for a fourth or higher use. The failure rate resets to 0 after a failed use.

## Quick Attack ##

* Advanced
* Standard
* Melee
* Physical
* Normal
* Accuracy 20
* 3 Damage Dice
* Quick Attack does not provoke reactions.

## Roar ##

* Advanced
* Full Turn
* Utility
* All enemy Pokemon in line of sight are pushed four hexes away in a straight line

## Sand Attack ##

* Basic
* Reaction
* Utility
* Ground
* Evade with double your evasion

## Scary Face ##

* Basic
* Standard
* Ranged
* Accuracy 18
* Concentration - Target speed drops to 0.

## Scratch ##

* Basic
* Standard
* Melee
* Physical
* Normal
* Accuracy 20
* 3 Damage Dice

## Self Destruct ##

* Basic
* Full Turn
* Ranged
* Physical
* Normal
* 8 Damage Dice
* Faints the user. Hits everyone within 4 hexes. Can not miss, and can not be evaded.

## Slash ##

* Advanced
* Standard
* Melee
* Physical
* Normal
* Accuracy 20
* 5 Damage Dice

## Smokescreen ##

* Basic
* Standard
* Utility
* Mark eight contiguous hexes centered on the user as smoke filled. Smoke blocks line of sight. Targets have 10 evasion against attacks, even if the evasion action is not used.

## Spider Web ##

* Advanced
* Full Turn
* Ranged
* Accuracy 20
* Target speed drops to 0.

## Sweet Scent ##

* Basic
* Reaction
* Utility
* React to a target in line of sight who uses evasion in response to an ally's attack. Target has their evasion increased to 18.

## Swords Dance ##

* Advanced
* Bonus
* Utility
* Concentration - Increase attack.

## Tackle ##

* Basic
* Standard
* Melee
* Physical
* Normal
* Accuracy 19
* 3 Damage Dice

## Tail Whip ##

* Advanced
* Reaction
* Melee
* Normal
* React during an attack against an opponent. Target defense is reduced.

## Taunt ##

* Advanced
* Standard
* Ranged
* Accuracy 20
* Concentration - Target may only use damaging attacks and evasion.

## Teleport ##

* Advanced
* Full Turn
* Utility
* Move to anywhere within 8 hexes.

## Thunderbolt ##

* Advanced
* Full Turn
* Ranged
* Special
* Electric
* Accuracy 20
* 6 Damage Dice
* 10% paralysis chance

## Uproar ##

* Basic
* Full Turn
* Ranged
* Special
* Normal
* Accuracy 20
* 4 Damage Dice
* Once, used this must be used each turn for 1d4 extra turns.

## Will-O-Wisp ##

* Advanced
* Full Turn
* Ranged
* Fire
* Accuracy 15
* Target is burned

## Wish ##

* Advanced
* Standard
* Utility
* Normal
* Concentration - Place a wish token on the board. At the end of your next turn, our nearest ally to the wish token recovers half their healh.
